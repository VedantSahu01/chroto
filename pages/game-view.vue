...
var selectedValues = ref([0, 0, 0, 0, 0]);
var actualValues = ref([1935, 1973, 1999, 2004, 2016]);
var finalPoints = useState("userPoints", () => [100, 100, 100, 100, 100]);
const imagesList = ref(["/images/test_img_1935.jpg",
  "/images/test_img_1973.jpg",
  "/images/test_img_1999.jpg",
  "/images/test_img_2004.jpg",
  "/images/test_img_2016.jpg",
]);
var displayedImage = ref("/images/test_img_1935.jpg");
var i = 0;
...
function gameLogic() {
  selectedValues.value[i] = displayValue.value;
  if (i >= 4) {
    const router = useRouter();
    for(var j =0; j< selectedValues.value.length; j++){
      finalPoints.value[j] -=  Math.abs(selectedValues.value[j] - actualValues.value[j]);
    }
    router.push('/result');
  } else {
    displayedImage.value = imagesList.value[i + 1];
    console.log(i);
    i++;
  }
}
...
<template>
  <div class="game-main-screen" @click=gameLogic()>
    <nuxt-img class="image-viewbox" :src=displayedImage fit="contain" />
